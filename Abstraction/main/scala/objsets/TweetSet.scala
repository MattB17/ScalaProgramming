...
abstract class TweetSet extends TweetSetInterface {
   ...
   * and be implemented in the subclasses?
   *
   * Answer: it should be implemented in the subclasses, union performs differently
   * for a Empty TweetSet vs a non-Empty Tweet Set.
   */
...
object GoogleVsApple {
  ...
  private def filterTweetsByTerms(twSet: TweetSet, terms: List[String]): TweetSet = {
    twSet.filter((tw: Tweet) => terms.exists(term => tw.text.contains(term)))
  }

  val allTweets = TweetReader.allTweets

  lazy val googleTweets: TweetSet = filterTweetsByTerms(allTweets, google)
  lazy val appleTweets: TweetSet = filterTweetsByTerms(allTweets, apple)
